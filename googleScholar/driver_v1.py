...
# Updated driver program that capture the number of citations per year. 
pg = ProxyGenerator()
...
def google_scholar_driver(input_dict):
    rows = []
    publications = []
    # Start from her 547/1390
    # 503/843
    for k,v in tqdm(list(input_dict.items())[1050:], desc = 'Processing ...'):
        try:
            ...
            pub_df.to_csv('google_scholar_education_publications.csv', encoding = 'utf-8')
            if len(publications) % 50 == 0:
                pickle_file = open('authors_ss.pkl', 'wb')
                pickle.dump(rows, pickle_file)
                pickle_file.close()
            ...
            logging.error('Author raised the following errors')
